...
export const EXCLUSION_FILTERS = [
  { id: 'no-played', label: 'No Played Games', keywords: [] },
  { id: 'no-horror', label: 'No Horror', keywords: ['horror', 'zombie', 'phasmophobia', 'resident evil', 'dead by daylight'] },
  { id: 'no-multiplayer', label: 'No Multiplayer', keywords: ['multiplayer', 'mmo', 'online', 'dota', 'counter-strike', 'cs:go', 'apex', 'overwatch', 'pubg', 'rainbow six', 'rocket league', 'destiny', 'dead by daylight', 'fall guys', 'sea of thieves', 'rust', 'tarkov', 'valorant', 'league of legends'] },
...
export function matchesExclusion(game, exclusionId) {
  ...
  // Special handler: exclude previously played games
  if (exclusionId === 'no-played') {
    return (game.playtime_forever || 0) > 0
  }
  ...
  if (ex.keywords.some(kw => nameLower.includes(kw))) return true

  if (exclusionId === 'no-multiplayer') {
    const moodTags = getGameMoodTags(game)
    if (moodTags.includes('coop')) return true
  }
...
export function estimateCompletionTime(game) {
  ...
  if (game.playtime_forever === 0) {
    if (baseHours <= 5) return 'Est: 3-6h'
    if (baseHours >= 70) return 'Est: 50-100h+'
    return 'Est: 15-30h'
  }
